...
import { expandVehiclePrices, routeComboHref, vehicleTypeSlug } from "@/types/route";
...
export async function generateStaticParams() {
  ...
  return routes.flatMap((route) => expandVehiclePrices(route.pricingByVehicle).map((vp) => ({ tinh: route.regionSlug || "khac", tuyen: route.slug, "loai-xe": vehicleTypeSlug(vp.vehicleType) })));
}
...
export default async function Page({ params }: Props) {
  ...
  const similarRoutes = allRoutes.filter((item) => item.regionSlug === route.regionSlug && item.slug !== route.slug && expandVehiclePrices(item.pricingByVehicle).some((price) => price.vehicleType === vp.vehicleType)).slice(0, 3);
