...
const STORAGE_KEY = "alo-dat-xe-admin-route-draft-v2";
            ...
            {step === 0 && (
              <>
                  ...
                  <label>
                    ...
                    <select value={draft.originId} onChange={(event) => updateDraft({ originId: event.target.value })}>
                      ...
                      {locations.map((location) => (
                        <option key={location.id} value={location.id} disabled={location.locked}>
                          {location.name} · {location.type}{location.locked ? " · Tạm khóa" : ""}
                        </option>
                      ))}
                    </select>
                  </label>
                ...
                {(origin?.locked || destination?.locked) && (
                  <InlineWarning>Long Thành đang PRELAUNCH nên chưa thể tạo hoặc sửa tuyến trong công cụ này.</InlineWarning>
                )}
                {existingRoute && (
