...
export function RouteResults({ routes, onClearFilters }: RouteResultsProps) {
  ...
  return <div className="route-results" aria-live="polite">{routes.map((route) => <article className="route-ticket" key={route.id}>
    ...
    <div className="rt-body"><div className="rt-route"><span>{getPublicLocationLabel(route.from)}</span><ArrowRight size={16} /><span>{getPublicLocationLabel(route.to)}</span></div><div className="rt-meta"><span><Clock3 size={13} /> {route.time}</span><span><Milestone size={13} /> {route.distance}</span><span className="rt-vehicles">{route.vehicleTypes.join(" · ")}</span></div></div>
    <div className="rt-cta"><Link href={routeHref(route)}>Xem chi tiết <ArrowRight size={14} /></Link></div>
  </article>)}</div>;
}
