...
export function ComboLandingPage({ route, vehiclePrice, category, similarRoutes, relatedPosts, vehicle }: {
  ...
  return (
    <main className="site-shell combo-page">
      <SiteHeader menuItems={navItems} hotline={SITE_HOTLINE} ctaLabel="Đặt xe ngay" ctaHref="#booking" />
      ...
      <section className="section-wrap combo-booking-section" id="booking">
        <div className="section-heading"><div><p className="section-label">XE PHÙ HỢP CHO HÀNH TRÌNH</p><h2>Chọn xe, đặt chuyến ngay.</h2></div><p className="heading-note">Giá và điều kiện chuyến được xác nhận trước khi khởi hành.</p></div>
        <article className="combo-vehicle-card">
          <div className="combo-vehicle-media">{image ? <MediaPhoto src={image} alt={vehicle?.name || category.label} /> : <CarFront size={76} strokeWidth={1.2} />}<strong>Xe {vehiclePrice.vehicleType}</strong></div>
          <div className="combo-vehicle-info">
            <div className="combo-vehicle-top"><div><h2>{vehicle?.name || `Xe ${vehiclePrice.vehicleType}`}</h2></div><div className="combo-price"><strong>{isContact ? "Liên hệ báo giá" : hasValidFixedPrice ? vehiclePrice.price : "Liên hệ báo giá"}</strong>{vehiclePrice.packageLabel && <small>{vehiclePrice.packageLabel}</small>}</div></div>
            <div className="combo-vehicle-divider" />
            {(vehicle?.seats || vehicle?.capacity) && <div className="combo-vehicle-details"><p><Users size={17} /> {vehicle.seats || vehicle.capacity}</p></div>}
            <div className="combo-vehicle-actions"><div><p className="combo-alert">{isContact || !hasValidFixedPrice ? "Liên hệ để xác nhận giá theo lịch thực tế" : "Giá và điều kiện chuyến được xác nhận trước khi khởi hành."}</p></div><RouteBookingActions route={routeLabel} routeId={route.id} vehicleType={vehiclePrice.vehicleType} price={hasValidFixedPrice ? vehiclePrice.price : undefined} direction="outbound" packageKey={vehiclePrice.packageKey} packageLabel={vehiclePrice.packageLabel} pricingMode={isContact || !hasValidFixedPrice ? "contact" : "fixed"} /></div>
          </div>
        </article>
      </section>

      <section className="section-wrap combo-benefits"><div className="section-heading"><div><p className="section-label">THÔNG TIN HÀNH TRÌNH</p><h2>Một chuyến đi nhẹ tênh.</h2></div></div><div className="combo-benefit-grid"><div><ShieldCheck size={22} /><strong>Thông tin rõ ràng</strong><p>Nhân viên xác nhận giá và điều kiện chuyến trước khi khởi hành.</p></div><div><Clock3 size={22} /><strong>Đón tận nơi</strong><p>Linh hoạt điểm đón tại {route.from} và trả khách tại {route.to}.</p></div><div><MessageCircle size={22} /><strong>Hỗ trợ nhanh</strong><p>Luôn có đội ngũ hỗ trợ qua điện thoại và Zalo.</p></div></div></section>
      ...
      <section className="vehicle-type-cta combo-final-cta"><div><p className="section-label">SẴN SÀNG LÊN ĐƯỜNG?</p><h2>Đặt xe {category.label.toLowerCase()} đi {route.to}.</h2><p>Nhân viên {SITE_NAME} sẽ xác nhận giá và điều kiện chuyến trước khi hoàn tất đặt xe.</p></div><a className="button button-primary" href={`tel:${SITE_HOTLINE_TEL}`} aria-label={`Gọi ${SITE_HOTLINE}`} >Gọi {SITE_HOTLINE} <Phone size={16} /></a></section>
      <SiteFooter tagline={<>Đi đâu cũng có {SITE_NAME}.<br />Kết nối những hành trình tử tế.</>} phone={SITE_HOTLINE} linkGroups={footerLinkGroups} socialLinks={defaultSocialLinks} copyright={`© 2026 ${SITE_NAME}`} madeFor="Made for the road." brandName={SITE_NAME} />
    </main>
  );
}
