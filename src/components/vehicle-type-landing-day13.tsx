...
import { ArrowRight, BusFront, Check, MapPin, PlaneTakeoff, ShieldCheck } from "lucide-react";
    ...
    <main className="site-shell vehicle-type-page">
      ...
      {galleryImages.length > 0 && (
        <section className="section-wrap vehicle-type-gallery-section">
          <div className="section-heading-row">
            <div>
              <p className="section-label">HÌNH ẢNH THỰC TẾ</p>
              <h2>Nhìn trước chuyến đi của bạn.</h2>
            </div>
          </div>
      ...
      {routePrices.length > 0 && (
        <section className="section-wrap vehicle-type-routes">
          <div className="section-heading-row">
            <div>
              <p className="section-label">GIÁ THAM KHẢO</p>
              <h2>Bảng giá theo tuyến phổ biến.</h2>
            </div>
      ...
      {relatedRoutes.length > 0 && (
        <section className="section-wrap vehicle-type-related-routes">
          <div className="section-heading-row">
            <div>
              <p className="section-label">TUYẾN CÓ THỂ ĐI</p>
              <h2>Đi đâu bằng {category.label.toLowerCase()}?</h2>
            </div>
          </div>
      ...
      {airportRoutes.length > 0 && (
        <section className="section-wrap vehicle-type-related-routes">
          <div className="section-heading-row">
            <div>
              <p className="section-label">TUYẾN SÂN BAY</p>
              <h2>Kết nối sân bay bằng {category.label.toLowerCase()}.</h2>
            </div>
          </div>
          <div className="departure-list">
            {airportRoutes.map((route) => (
              <Link key={route.href} href={route.href} className="vehicle-chip">
                <PlaneTakeoff size={15} /> {route.label}
              </Link>
            ))}
          </div>
        </section>
        ...
        <section className="section-wrap vehicle-type-related-posts">
          <div className="section-heading-row">
            <div>
              ...
              <h2>Thêm cảm hứng cho hành trình.</h2>
            </div>
            <Link href="/blog" className="text-link">Xem tất cả bài viết <ArrowRight size={15} /></Link>
          </div>
      ...
      {otherCategories.length > 0 && (
        <section className="section-wrap vehicle-type-others">
          <div className="section-heading-row">
            <div>
              <p className="section-label">LOẠI XE KHÁC</p>
              <h2>Xem thêm các loại xe khác.</h2>
            </div>
            ...
            {otherCategories.map((other) => (
              <Link href={`/loai-xe/${other.slug}`} className="vehicle-category-card" key={other.slug}>
                ...
                <div className="vehicle-category-card-body">
                  <div className="vehicle-category-card-heading">
                    <p>{other.label}</p>
                    <span>{other.type}</span>
                  </div>
                  <h3>{other.title}</h3>
                  <p className="vehicle-category-summary">{other.shortDescription}</p>
                  <div className="vehicle-category-card-meta">
                    <span className="vehicle-category-badge">{other.driveOptions.join(" / ")}</span>
                    <strong>{other.startingPrice}</strong>
                  </div>
...
export function VehicleCategoryIndex({ categories, routes }: { categories: VehicleCategory[]; routes: Route[] }) {
  return (
    <main className="site-shell vehicle-type-page">
      <SiteHeader menuItems={navItems} hotline={SITE_CONTACT_PHONE_DISPLAY} ctaLabel="Thuê xe ngay" ctaHref="/#booking" />
      <SubpageHero
        title="Mỗi hành trình, một lựa chọn vừa vặn."
        description="Khám phá các nhóm xe được thiết kế cho từng kiểu chuyến đi — từ gia đình nhỏ đến đoàn lớn."
          ...
          {categories.map((category) => (
            <Link href={`/loai-xe/${category.slug}`} className="vehicle-category-card" key={category.slug}>
              ...
              <div className="vehicle-category-card-body">
                ...
                <div className="vehicle-category-card-meta">
                  <span className="vehicle-category-badge">{category.driveOptions.join(" / ")}</span>
